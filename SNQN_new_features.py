...
###### Modified the final_score with item features#########
            ...
            self.phi_score = tf.matmul(self.states_hidden, self.hidden_feature, transpose_b=True)

            self.final_score = (1 - self.lambda_) * self.output2 + self.lambda_ * self.phi_score                                               
...
if __name__ == '__main__':
    ...
    item_feature = pd.read_csv(os.path.join(data_directory, "item_onehotencode_new.csv")).values.astype('float32')
    ...
    QN_1 = QNetwork(name='QN_1', hidden_size=args.hidden_factor, learning_rate=args.lr, item_num=item_num,
                    state_size=state_size, pretrain=False,feature_vec = item_feature)
